...
export interface IngestReport {
  ...
  flagged?: number;
  /** Applications whose marks are stored with no model total yet to compare them against. */
  awaiting_scores?: number;
 ...
 *
 * The last sentence is about the comparison, and there isn't one until the cohort has been scored.
 * Saying "0 flagged" there reads as "nothing disagrees", which is the opposite of what an unscored
 * cohort means — so an unscored cohort is told it is waiting instead.
 */
export function reportSummary(report: IngestReport): string {
  ...
  const awaiting = report.awaiting_scores ?? 0;
  return (
    ...
    (awaiting > 0
      ? ` ${awaiting} of them have no model score yet, so there is nothing to compare them` +
        ` against — their gaps will show once the cohort is scored.`
